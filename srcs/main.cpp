...
int 	main(int ac, char **av)
{
	...
	if (ac != 2)
		return (ret_error("Usage: ./webserv webserv-file"));
	try
	{
		webserv.parse(av[1], servers);
		webserv.init(&rSet, &wSet, &readSet, &writeSet, &timeout);
	}
	...
	while (isServerRunning)
	{
		readSet = rSet;
		writeSet = wSet;
		select(webserv.getMaxFd(servers) + 1, &readSet, &writeSet, NULL, &timeout);
		for (std::vector<Server>::iterator s(servers.begin()); s != servers.end(); ++s)
		{
			if (FD_ISSET(s->getFd(), &readSet))
			{
				try
				{
					if (!isServerRunning)
						break ;
					if (webserv.getOpenFd(servers) > maxFD)
						s->refuseConnection();
					else
						s->acceptConnection();
				}
