...
extern	std::vector<Webserv> servers;
extern	bool				g_state;
...
void			Config::parse(char *file, std::vector<Webserv> &servers)
{
	size_t					d;
	size_t					nb_line;
	std::string				context;
	...
	while (!buffer.empty())
	{
		...
		if (!line.compare(0, 6, "server"))
		{
			...
			if (!line.compare(0, 7, "server{"))
			{
				...
				std::vector<Webserv>::iterator it(servers.begin());
				while (it != servers.end())
				{
					if (tmp["server|"]["listen"] == it->s_config.back()["server|"]["listen"])
					{
						if (tmp["server|"]["server_name"] == it->s_config.back()["server|"]["server_name"])
							throw(Config::InvalidConfigFileException(nb_line));
						else
							it->s_config.push_back(tmp);
						break ;
					}
					++it;
				}
				if (it == servers.end())
				{
					server.s_config.push_back(tmp);
					servers.push_back(server);
				}
				server.s_config.clear();
				tmp.clear();
				context.clear();
			}
			else
...
int				Config::getMaxFd(std::vector<Webserv> &servers)
{
	int		max = 0;
	int		fd;
...
int				Config::getOpenFd(std::vector<Webserv> &servers)
{
	int		nb = 0;
